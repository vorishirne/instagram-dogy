...
class clippo extends CustomClipper<Rect> {
  Rect getClip(Size size) {
    return Rect.fromLTWH(5, 5, 38, 38);
  }
...
class AuthCardState extends State<AuthCard> with TickerProviderStateMixin {
  ...
  void _switchRecovery(bool recovery) async {
    final auth = Provider.of<Auth>(context, listen: false);

    auth.isRecover = recovery;

    if (recovery) {
      String error = await auth.onRecoverPassword(auth.email);
  ...
  Future<void> _forwardChangeRouteAnimation() {
    ...
    final cardSize = getWidgetSize(_ncardkey);
    _routeTransitionController.forward();
    // add .25 to make sure the scaling will cover the whole screen
    final widthRatio =
        0; //deviceSize.width / cardSize.width + (isLogin ? .25 : .65);
    final heightRatio = 0; //deviceSize.height / cardSize.height + .25;

    _cardSize2AnimationX =
        Tween<double>(begin: 1.0, end: heightRatio / cardSizeScaleEnd)
            .animate(CurvedAnimation(
      parent: _routeTransitionController,
      curve: Interval(.72727272, 1, curve: Curves.bounceIn),
    ));
    _cardSize2AnimationY =
    ...
    return Visibility(
        ...
        child: FadeIn(
          ...
          child: FlatButton(
            child: AnimatedText(
              text:
                  auth.isSignup ? messages.loginButton : messages.signupButton,
...
class _RecoverCardState extends State<_RecoverCard>
  ...
  Future<bool> _submit() async {
    final auth = Provider.of<Auth>(context, listen: false);
    //auth.password=_nameController.text;
    if (!_formRecoverKey.currentState.validate()) {
      return false;
    }
    _formRecoverKey.currentState.save();
...
//    }
  ...
  Widget _buildBackButton(ThemeData theme, LoginMessages messages) {
    return FlatButton(
      ...
      padding: EdgeInsets.symmetric(horizontal: 30.0, vertical: 4),
      materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
      textColor: Color.fromRGBO(24, 115, 172, .5),
    );
  }
